...
import { DeadlinePill, Pill } from "@/components/ui/badge";
import { useLifeStore } from "@/lib/life-store";
import { laterTasks } from "@/lib/queue";
    ...
    <div className="space-y-4">
      ...
      {later.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-surface px-5 py-10 text-center">
          <h2 className="font-display text-2xl text-fg">Nothing waiting</h2>
      ...
      <button type="button" className="w-full text-left" onClick={onOpen}>
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <h3 className="mt-1 font-display text-lg text-fg">{task.title}</h3>
          </div>
