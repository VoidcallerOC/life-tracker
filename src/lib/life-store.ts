...
import type { Animal, Client, LifeSnapshot, Status, Task, UndoEntry } from "./types";
import { emptyAnimal, emptyClient, emptyTask, normalizeClient, rid } from "./types";
import { starterSnapshot } from "./seed";
import { addDays, todayIso } from "./dates";
import { recomputeAnimalDue } from "./guardrails";
...
function mergeSnapshots(persisted: unknown, current: LifeStore): LifeStore {
  if (!persisted || typeof persisted !== "object") return current;
  const p = persisted as Partial<LifeSnapshot> & { undoStack?: UndoEntry[] };
  const existingClients = Array.isArray(p.clients) ? p.clients.map((c) => normalizeClient(c)) : [];
  const byId = new Map(existingClients.map((c) => [c.id, c]));
  for (const seed of current.clients) {
    if (!byId.has(seed.id)) byId.set(seed.id, seed);
  }
  const existingTasks = Array.isArray(p.tasks) ? p.tasks : [];
  const tById = new Map(existingTasks.map((t) => [t.id, t]));
  for (const seed of current.tasks) {
    if (!tById.has(seed.id)) tById.set(seed.id, seed);
  }
  return {
    ...current,
    clients: [...byId.values()],
    animals: Array.isArray(p.animals) ? p.animals : current.animals,
    tasks: [...tById.values()],
    coachDismissed: Boolean(p.coachDismissed),
    undoStack: Array.isArray(p.undoStack) ? p.undoStack.slice(-15) : [],
  };
}

const seed = starterSnapshot();
...
export const useLifeStore = create<LifeStore>()(
  persist(
    (set, get) => ({
      ...
      resetToStarter: () => {
        const current = get();
        const next = starterSnapshot();
        set({
          ...next,
          undoStack: [
            ...current.undoStack,
            { label: "Reset to starter pack", at: Date.now(), snapshot: snapshotOf(current) },
          ].slice(-15),
        });
      },
      addClient: (partial) => {
    ...
    {
      name: "nicklife-os-v1",
      merge: (persisted, current) => mergeSnapshots(persisted, current),
      partialize: (s) => ({
        clients: s.clients,
        animals: s.animals,
        tasks: s.tasks,
        coachDismissed: s.coachDismissed,
        undoStack: s.undoStack.slice(-8),
      }),
    },
  ),
);
