...
import { StatusPill } from "@/components/os/ui/badge";
import { ContactActions } from "@/components/os/contact-actions";
import { useLifeStore } from "@/lib/os/store";
import { CARE_PLAN, STALE_AFTER_DAYS } from "@/lib/os/guardrails";
import { formatMoney } from "@/lib/os/money";
import { addDays, daysUntil, todayIso } from "@/lib/os/dates";
import type { Client, Status } from "@/lib/os/types";
            ...
            <li key={c.id} className="rounded-xl border border-border bg-surface p-4">
              <button type="button" className="w-full text-left" onClick={() => onOpen(c.id)}>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="text-base font-semibold leading-snug tracking-tight text-fg">{c.name}</h3>
